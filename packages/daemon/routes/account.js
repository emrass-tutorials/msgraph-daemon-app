...
router.get("/requestpermissions", async function (req, res) {
  ...
  res.redirect(
    `https://login.microsoftonline.com/${tenantId}/adminconsent?client_id=${process.env.OAUTH_APP_ID}&redirect_uri=${process.env.OAUTH_ADMINCONSENT_REDIRECT_URI}&scope=https://graph.microsoft.com/.default`
  );
});
